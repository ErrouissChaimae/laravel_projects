...
@section('content')
 ...
 <div class="container">
    ...
    <div class="col-sm-4">
        <form action="{{ route('autocar.search') }}" method="GET" class="mb-3">
            <div class="input-group">
                <input type="text" name="query" class="form-control" placeholder="Search by Name autocar" required style="border-radius: 20px">
...
<a href="{{ route('autocars.create') }}" class="btn btn-success"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-lg" viewBox="0 0 16 16">
        ...
        <table class="table">
            ...
            <tbody>
                @foreach ($autocars as $autocar)
                    <tr>
                        <td><img src="{{ asset('images/' . $autocar->photo) }}" alt="{{ $autocar->nom }}" width="100"></td>
                        <td>{{ $autocar->nom }}</td>
                        <td>{{ $autocar->nombre_de_places }}</td>
                        <td>
                            <form action="{{ route('autocars.destroy', $autocar->id_autocar) }}" method="POST" style="display:inline;"> 
                                @csrf 
                                @method('DELETE') 
                                <a href="{{ route('autocars.edit', $autocar->id_autocar) }}"  class="edit-icon"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
